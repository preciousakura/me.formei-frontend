...
export function DisciplineCard({ data }: DisciplineCardProps) {
    ...
    >
      <Container>
        <VStack space={1}>
          ...
          <Subtitle color={theme.colors.text} size={19}>{data.name}</Subtitle>
            ...
            {data.prerequisites.length > 0
              ? data.prerequisites.map((pr, i) => (
                  <H5 key={`${pr.cod}_${i}`} color={theme.colors.primary[500]}>
                    {pr.name}{i !== data.prerequisites.length - 1 && ", "}
                  </H5>
                ))
              : "-"}
          </H5>
